...
namespace Web_API_Demo.Tests.Common.Helpers
{
    public class FakeDbSet<T> : EnumerableQuery<T>, IDbSet<T> where T : class, new()
    {
        readonly ObservableCollection<T> _collection;

        private static ObservableCollection<T> CreateDefaultCollection(IEnumerable<T> enumarable)
        {
            return enumarable != null ? new ObservableCollection<T>(enumarable) : new ObservableCollection<T>();
        }
